# Stack Internal (Enterprise) Webhook Report
A Python script for Stack Internal (Enterprise) that creates a CSV report of configured webhooks. You can see an [example](https://github.com/StackExchange/so4t_WebHook_report/blob/main/Examples/webhooks.csv) of the output in the Examples directory.

## Requirements
* Stack Internal (Enterprise) and a user account with admin permissions
...
At the beginning of the script, a small Chrome window will appear, prompting you to log in to your instance of Stack Internal (Enterprise). After logging in, the Chrome window will disappear, and the script will proceed in the terminal window.
...
All data obtained via this script is handled locally on the device from which the script is run. The script does not transmit data to other parties, such as Stack Overflow. This script is entirely read-only, and you cannot change/add content to your Stack Internal instance.